...
def solve_ocean(R):
    ...
    with tf.device('/gpu:%s' % (R['gpuNo'])):
        with tf.compat.v1.variable_scope('vscope', reuse=tf.compat.v1.AUTO_REUSE):
            ...
            UNN2train, loss_it = model.loss2PDE(X=X_in, t=t_in, loss_type=R['loss_type'])

            loss_bd2left = model.loss2bd(X_bd=Xbd_left, t=t_bd, Ubd_exact=Ubd_left, if_lambda2Ubd=False)
...
if __name__ == "__main__":
    ...
    R['learning_rate_decay'] = 1e-3  # 学习率 decay
    # R['learning_rate_decay'] = 5e-4  # 学习率 decay
